...
Route::middleware(['auth', App\Http\Middleware\LicenseGate::class, App\Http\Middleware\EnsureSubscriptionActive::class, App\Http\Middleware\DemoWriteGuard::class, App\Http\Middleware\EditionGuard::class])->group(function () {
    ...
    // P0 Foundation — in-app notification bell / Notifications screen feed.
    Route::get('/app/notifications/feed', [App\Http\Controllers\NotificationController::class, 'feed'])->name('app.notif.feed');
    Route::post('/app/notifications/read-all', [App\Http\Controllers\NotificationController::class, 'markAllRead'])->name('app.notif.readall');
    Route::post('/app/notifications/{id}/read', [App\Http\Controllers\NotificationController::class, 'markRead'])->whereNumber('id')->name('app.notif.read');
    // F8 Greetings — config, preview, test-send, delivery log.
    Route::get('/app/greetings/config', [App\Http\Controllers\GreetingController::class, 'config'])->name('app.greetings.config');
    Route::post('/app/greetings/config', [App\Http\Controllers\GreetingController::class, 'saveConfig'])->name('app.greetings.config.save');
    Route::post('/app/greetings/preview', [App\Http\Controllers\GreetingController::class, 'preview'])->name('app.greetings.preview');
    Route::post('/app/greetings/test', [App\Http\Controllers\GreetingController::class, 'test'])->name('app.greetings.test');
    Route::get('/app/greetings/log', [App\Http\Controllers\GreetingController::class, 'log'])->name('app.greetings.log');
    // F6/F7 Probation — board, config, confirm, extend, history.
    Route::get('/app/probation/board', [App\Http\Controllers\ProbationController::class, 'board'])->name('app.probation.board');
    Route::get('/app/probation/config', [App\Http\Controllers\ProbationController::class, 'config'])->name('app.probation.config');
    Route::post('/app/probation/config', [App\Http\Controllers\ProbationController::class, 'saveConfig'])->name('app.probation.config.save');
    Route::post('/app/probation/{emp}/confirm', [App\Http\Controllers\ProbationController::class, 'confirm'])->whereNumber('emp')->name('app.probation.confirm');
    Route::post('/app/probation/{emp}/extend', [App\Http\Controllers\ProbationController::class, 'extend'])->whereNumber('emp')->name('app.probation.extend');
    Route::get('/app/probation/{emp}/history', [App\Http\Controllers\ProbationController::class, 'history'])->whereNumber('emp')->name('app.probation.history');
    // F3 Dashboard — the logged-in employee's today attendance panel.
    Route::get('/app/today', [App\Http\Controllers\TodayController::class, 'me'])->name('app.today');
    // F10 Previous-day absence — config, manual run, delivery log.
    Route::get('/app/absence/config', [App\Http\Controllers\AbsenceController::class, 'config'])->name('app.absence.config');
    Route::post('/app/absence/config', [App\Http\Controllers\AbsenceController::class, 'saveConfig'])->name('app.absence.config.save');
    Route::post('/app/absence/run', [App\Http\Controllers\AbsenceController::class, 'run'])->name('app.absence.run');
    Route::get('/app/absence/log', [App\Http\Controllers\AbsenceController::class, 'log'])->name('app.absence.log');
    // F2 Attendance correction (regularisation) — employee raise/cancel, HR review.
    /*
    | F1 — Statutory Configuration: scoped (company / state / tenant),
    | effective-dated rate overrides, plus the editable PT state slabs.
    | Every route is role-guarded inside the controller via
    | FeaturePermissions::guard('statutory.config').
    */
    Route::get('/app/statutory-config', [App\Http\Controllers\StatutoryConfigController::class, 'boot'])->name('app.statcfg.boot');
    Route::get('/app/statutory-config/effective', [App\Http\Controllers\StatutoryConfigController::class, 'effective'])->name('app.statcfg.effective');
    Route::post('/app/statutory-config/save', [App\Http\Controllers\StatutoryConfigController::class, 'save'])->name('app.statcfg.save');
    Route::post('/app/statutory-config/preview', [App\Http\Controllers\StatutoryConfigController::class, 'preview'])->name('app.statcfg.preview');
    Route::post('/app/statutory-config/slab', [App\Http\Controllers\StatutoryConfigController::class, 'saveSlab'])->name('app.statcfg.slab');

    Route::get('/app/attendance-corrections', [App\Http\Controllers\AttendanceCorrectionController::class, 'index'])->name('app.attcorr.index');
    Route::get('/app/attendance-corrections/day', [App\Http\Controllers\AttendanceCorrectionController::class, 'day'])->name('app.attcorr.day');
    Route::post('/app/attendance-corrections', [App\Http\Controllers\AttendanceCorrectionController::class, 'store'])->name('app.attcorr.store');
    Route::post('/app/attendance-corrections/{id}/cancel', [App\Http\Controllers\AttendanceCorrectionController::class, 'cancel'])->whereNumber('id')->name('app.attcorr.cancel');
    Route::post('/app/attendance-corrections/{id}/decide', [App\Http\Controllers\AttendanceCorrectionController::class, 'decide'])->whereNumber('id')->name('app.attcorr.decide');
    // F9 Employee import wizard — upload/preview/commit/history/error report.
    Route::post('/app/employee-import/upload', [App\Http\Controllers\EmployeeImportController::class, 'upload'])->name('app.empimport.upload');
    Route::post('/app/employee-import/preview', [App\Http\Controllers\EmployeeImportController::class, 'preview'])->name('app.empimport.preview');
    Route::post('/app/employee-import/commit', [App\Http\Controllers\EmployeeImportController::class, 'commit'])->name('app.empimport.commit');
    Route::get('/app/employee-import/history', [App\Http\Controllers\EmployeeImportController::class, 'history'])->name('app.empimport.history');
    Route::get('/app/employee-import/{id}/errors.csv', [App\Http\Controllers\EmployeeImportController::class, 'errorReport'])->whereNumber('id')->name('app.empimport.errors');
